...
import PrintRawMat from './PrintRawMat';
...
const RawMaterial = () => {
    ...
    const moduleName = 'QC Raw Material';
    ...
    const handleTable = () => {
        router.push("/qualitycontrol/qctest/rawmaterial/rawmattable");
    };

    const handleExit = async () => {
        router.push("/qualitycontrol/?activeTab=qc-test");
    };
    ...
    const handlePrint = async () => {
        try {
            ...
            if (STATUS === 0 && Array.isArray(DATA)) {
                ...
                const asPdf = pdf(<PrintRawMat rows={formattedData} />);
    ...
    return (
        <Grid container sx={{ bgcolor: '#fffff', py: 4 }}>
            <ToastContainer />
