...
const PrintProdSrDt = ({ rows, currentCatData }) => {
      ...
      {Array.from({ length: Math.ceil(rows.length / 30) }).map((_, pageIndex) => (
        <Page key={pageIndex} size="A4" style={{ padding: 30, fontSize: 10, fontFamily: 'Helvetica', position: 'relative' }}>
          ...
          <View style={{ position: 'absolute', top: 20, left: 30, right: 30, textAlign: 'center', marginBottom: 15 }} fixed>
            ...
            <Text style={{ fontSize: 14, fontWeight: 'bold', marginBottom: 5 }}>Product Series MASTER REPORT</Text>
          </View>
          ...
          <View style={{ marginTop: 30, marginBottom: 20 }}>
            ...
            <View style={{ border: '1px solid #000', borderRadius: 3, marginBottom: 10 }}>
              ...
              <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#000', borderBottomStyle: 'solid', borderTopWidth: 1, borderTopColor: '#000', borderTopStyle: 'solid', backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>
                ...
                <Text style={{ width: '15%', borderRight: '1px solid #ddd', padding: 5 }}>Series Key</Text>
                <Text style={{ width: '50%', borderRight: '1px solid #ddd', padding: 5 }}>Name</Text>
                <Text style={{ width: '15%', borderRight: '1px solid #ddd', padding: 5 }}>Mrp</Text>
              ...
              {rows.slice(pageIndex * 30, (pageIndex + 1) * 30).map((row, index) => (
                <View key={row.PRODGRP_KEY} style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#ddd', borderBottomStyle: 'solid' }}>
                  ...
                  <Text style={{ width: '15%', borderRight: '1px solid #ddd', padding: 5 }}>{row.FGPRD_KEY}</Text>
                  <Text style={{ width: '50%', borderRight: '1px solid #ddd', padding: 5 }}>{row.NAME}</Text>
                  <Text style={{ width: '15%', borderRight: '1px solid #ddd', padding: 5 }}>{row.Mrp}</Text>
