...
import z from 'zod';
// import { PrintQualityDt } from './PrintQualityDt';
...
const Quality = () => {
    ...
    const fetchRetriveData = async (currentQUALITY_KEY, flag = "R", isManualSearch = false) => {
        try {
    ...
    useEffect(() => {
        if (location.state && location.state.QLTY_KEY) {
            setCurrentQUALITY_KEY(location.state.QLTY_KEY);
            fetchRetriveData(location.state.QLTY_KEY);
    ...
    const handlePrint = async () => {
        try {
            const response = await axiosInstance.post(
                `QUALITY/GetQUALITYDashBoard?currentPage=1&limit=5000`,
                { SearchText: "" }
            );
    ...
    const handleExit = () => { 
        // Router.push("/masters/products/quality-mst-table")
    };
