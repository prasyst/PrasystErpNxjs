...
import {
    MdAdd, MdPerson, MdList, MdCheckCircleOutline, MdWarning, MdRefresh, MdAnalytics, MdSchedule,
...
import {
    ...
    Divider, CircularProgress, Container, IconButton, useTheme, useMediaQuery, Avatar, Tooltip, Fade, LinearProgress, alpha, ToggleButton,
    ToggleButtonGroup, MenuItem, Select, FormControl
} from '@mui/material';
import {
    Add as AddIcon,
        ...
        try {
            ...
            if (response.data.STATUS === 0 && Array.isArray(response.data.DATA)) {
                const myTickets = response.data.DATA.map(tkt => ({
                    ...
                    Category: tkt.TKTCATNAME || "NA",
                    SubCat: tkt.TKTSUBCATNAME || "NA",
                    ResRemark: tkt.RSLVRMRK || "NA",
                    TktDt: tkt.TKTDATE || "NA",
                    Service: tkt.TKTSERVICENAME || "NA",
                    TicketTag: tkt.TKTTAGNAME || "NA",
            ...
            >
                <Box sx={{
                    ...
                    <Box sx={{ flex: 1 }}>
                        ...
                        >
                            Manage and track your all support tickets
                        </Typography>
                    </Box>
                    ...
                    >
                        ...
                        <Button size='small'
                            variant="contained"
                            startIcon={<AddIcon />}
                            sx={{ bgcolor: '#635bff', ":hover": { bgcolor: '#635bff' }, height: 35, textTransform: 'none' }}
                            onClick={() => router.push('/emp-tickets/create-tickets')}
                        >
                            {isMobile ? 'New' : 'New Ticket'}
                        </Button>
                        <IconButton sx={{ height: 35, width: 35, bgcolor: '#635bff', ":hover": { bgcolor: '#635bff' }, color: '#fff' }}
                            onClick={() => router.back()}
                        >
                            <ArrowBackIcon />
                        </IconButton>
