...
import PrintPincode from './PrintPincode';
...
const PinCodeMst = () => {
  ...
  const handleTable = () => {
    router.push('/masters/location/pincode/pincodetable');
  };
  ...
  const handlePrint = async () => {
    try {
      ...
      if (STATUS === 0 && Array.isArray(DATA)) {
        ...
        const asPdf = pdf(<PrintPincode rows={formattedData} />);
  ...
  const handleExit = () => {
    router.push('/masterpage/?activeTab=59');
  };
