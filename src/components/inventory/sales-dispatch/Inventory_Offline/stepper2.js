...
const handleConfirmAdd = () => {
  ...
  const newItems = selectedShades.map((shade, shadeIndex) => {
    ...
    const dbFlag = mode === 'add' ? 'I' : 'I'; // New items always get 'I'
...
originalData: {
  ...
  MRP: mrp,
  ITMRATE: rate,
  AMT: totalAmount,  // ADD THIS LINE - this is important for GST calculation
  ITMAMT: totalAmount,  // Keep for backward compatibility
  DLV_VAR_PERC: parseFloat(newItemData.varPer) || 0,
  DLV_VAR_QTY: 0,
  DISC_AMT: discount,
  NET_AMT: netAmount,
  ...
  DBFLAG: dbFlag
      ...
      FGPTN_KEY: fgptnKey,
      STYCATRT_ID: stycatrtId,
      dbFlag: dbFlag // Add dbFlag to the row for reference
    };
  });
  ...
  const finalNewItems = selectedShades.length > 0 ? newItems : [{
    ...
    originalData: {
      ...
      FGPTN_KEY: lotNoMapping[newItemData.lotNo] || "",
      STYCATRT_ID: stycatrtId,
      DBFLAG: mode === 'add' ? 'I' : 'I' // New items always get 'I'
    },
    FGSTYLE_ID: fgstyleId,
    FGPRD_KEY: fgprdKey,
    FGTYPE_KEY: typeMapping[newItemData.type] || "",
  ...
  // Update table data - KEEP EXISTING ITEMS AND ADD NEW ONES
  const existingTableData = tableData.filter(row => 
    // Keep rows that are either:
    // 1. Existing items from API (have ORDBKSTY_ID > 0 and not marked for deletion)
    // 2. Or new items already added in this session
    (row.originalData?.ORDBKSTY_ID > 0 && row.originalData?.DBFLAG !== 'D') ||
    (row.originalData?.ORDBKSTY_ID > 999999) // Temporary IDs from this session
  );
  
  const newTableData = [...existingTableData, ...finalNewItems];
  setUpdatedTableData(newTableData);

  // Update formData with ALL items (existing + new)
  // Get existing ORDBKSTYLIST items from formData (with proper DBFLAG)
  const existingOrdbkStyleItems = (formData.apiResponseData?.ORDBKSTYLIST || [])
    .filter(item => item.DBFLAG !== 'D') // Exclude deleted items
    .map(item => {
      // Ensure existing items have DBFLAG 'U' for update
      return {
        ...item,
        DBFLAG: 'U' // Existing items get 'U' in edit mode
      };
    });

  // Create new ORDBKSTYLIST items from finalNewItems
  const newOrdbkStyleItems = finalNewItems.map(item => ({
    ORDBKSTY_ID: item.id,
    FGITEM_KEY: item.BarCode,
    PRODUCT: item.product,
    ...
    ORDBKSTYSZLIST: updatedSizeDetails.map(size => ({
      ...size,
      ORDBKSTYSZ_ID: 0,
      DBFLAG: 'I' // Size details for new items get 'I'
    })),
    FGSTYLE_ID: item.FGSTYLE_ID,
    FGPRD_KEY: item.FGPRD_KEY,
    FGTYPE_KEY: item.FGTYPE_KEY,
    ...
    DBFLAG: 'I' // New items get 'I' in edit mode
  }));

  // Combine existing and new items
  const combinedOrdbkStyleItems = [...existingOrdbkStyleItems, ...newOrdbkStyleItems];

  console.log('Combined ORDBKSTYLIST:', combinedOrdbkStyleItems.map(item => ({
  ...
  setFormData(prev => ({
    ...
    apiResponseData: {
      ...prev.apiResponseData,
      ORDBKSTYLIST: combinedOrdbkStyleItems
    }
  }));
